...
class Lunar
{
	...
	//字符串类型
	public  $weekName;	//星期名称
	public  $yearName;	//年名称
	public  $monthName;	//月名称
	public  $dayName;	//日名称
	public  $shengxiao;	//生肖
	/**
	 +----------------------------------------------------------
	...
	public function getDateString()
	{
		...
		//返回字符串
		return $strday . ' ' . $strlday . ' 星期' . $strweek;
	}

	//内部辅助函数，获取字符串变量
	private function fillStringVar()
	{
		//字符串类型
		$this->weekName = $this->magicInfo('WN');	//星期名称
		$this->yearName = $this->magicInfo('GZ');	//年名称
		$this->monthName = $this->magicInfo('MN');	//月名称
		$this->dayName = $this->magicInfo('DN');	//日名称
		$this->shengxiao = $this->magicInfo('SX');	//生肖
	}
